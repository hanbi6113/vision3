// app/components/discover/discover-watch-shell.tsx
...
export default function DiscoverWatchShell({ title, returnTo }: Props) {
  ...
  const currentDiscoverPath = `/discover/watch/${title.slug}?returnTo=${encodeURIComponent(
    returnTo
  )}`;

  const castPageHref = `/watch/${title.slug}/cast?episode=1&returnTo=${encodeURIComponent(
    currentDiscoverPath
  )}`;

  const premiumWatchHref = `/watch/${title.slug}?episode=1&returnTo=${encodeURIComponent(
    currentDiscoverPath
  )}`;

  const discoverHallHref = `/discover/${title.genre}`;
            ...
            >
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {discoverRecommendations.map((item) => (
                  <Link
                    key={item.slug}
                    href={`/discover/watch/${item.slug}?returnTo=${encodeURIComponent(
                      currentDiscoverPath
                    )}`}
