// components/ott/ott-1-shell.tsx
...
export default function OttOneShell({ activeGenre }: Props) {
      ...
      <div className="mx-auto max-w-7xl px-5 py-8 md:px-8 lg:px-10">
        ...
        >
          <div>
            ...
            <p className={`mt-4 max-w-3xl text-sm leading-7 sm:text-base ${bodyColorClass}`}>
              Discover와 같은 장르 흐름을 유지하되, 더 프리미엄한 스트리밍 서비스처럼
              대표작과 큐레이션 중심으로 정리한 OTT 메인 시안이야.
            </p>
          </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.32fr_0.68fr]">
          {/* hero */}
          <div
            className={`h-full overflow-hidden rounded-[34px] border backdrop-blur-xl ${cardShellClass}`}
          >
            <div
          ...
          <div className="grid gap-5">
            <div className={`rounded-[30px] border p-6 backdrop-blur-xl ${cardShellClass}`}>
              <p className={`text-xs font-bold uppercase tracking-[0.26em] ${accentColorClass}`}>
                Featured Character
              </p>
        ...
        <section className="pt-10">
          <div className="grid gap-5 md:grid-cols-3">
            {content.stats.map((stat) => (
              <article
                key={stat.label}
                className={`rounded-[28px] border p-6 backdrop-blur-xl ${cardShellClass}`}
              >
        ...
        <section className="pt-10">
          <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
            <div
              className={`overflow-hidden rounded-[34px] border backdrop-blur-xl ${cardShellClass}`}
            >
            ...
            <div className="grid gap-5">
              {content.notes.map((note) => (
                <article
                  key={note.title}
                  className={`rounded-[30px] border p-6 backdrop-blur-xl ${cardShellClass}`}
                >
                  <p className={`text-xs font-bold uppercase tracking-[0.24em] ${accentColorClass}`}>
                    {note.label}
                  </p>
        ...
        <section className="pb-10 pt-10">
          <div className="mb-5 flex items-end justify-between">
            <h2 className={`text-3xl font-extrabold ${titleColorClass}`}>Top Picks</h2>
          </div>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {expandedCards.map((item, index) => (
              <article
                ...
                className={`overflow-hidden rounded-[28px] border ${cardShellClass}`}
              >
                <div className="relative h-[280px]">
                  ...
                  />
                  {(isSf || isMurim || isHorror) && (
                    <div
                      className={`absolute inset-0 ${
                        isSf
                          ? "bg-[linear-gradient(to_top,rgba(3,10,18,0.72)_8%,rgba(3,10,18,0.12)_45%,rgba(3,10,18,0.02)_100%)]"
                          : isMurim
                          ? "bg-[linear-gradient(to_top,rgba(244,240,228,0.76)_8%,rgba(244,240,228,0.14)_45%,rgba(244,240,228,0.04)_100%)]"
                          : "bg-[linear-gradient(to_top,rgba(7,7,7,0.78)_8%,rgba(7,7,7,0.20)_45%,rgba(7,7,7,0.04)_100%)]"
                      }`}
                    />
                  )}
                ...
                <div className="p-6">
                  ...
                  <h3 className={`mt-2 text-2xl font-extrabold ${titleColorClass}`}>
                    {item.title}
                  </h3>
                  <p className={`mt-3 text-sm leading-7 ${bodyColorClass}`}>{item.desc}</p>
